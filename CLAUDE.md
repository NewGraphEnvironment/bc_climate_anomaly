...
R Shiny app visualizing monthly, seasonal, and annual climate anomalies for British Columbia. Originally developed by BC Ministry of Forests (FFEC) using ERA5-Land reanalysis data against a 1981-2010 baseline.

## Fork Status

- **Upstream:** `bcgov/bc_climate_anomaly` (BC Government)
- **Fork:** `NewGraphEnvironment/bc_climate_anomaly`
- **Branch:** `main` (synced with upstream)
- **Archive:** `aoi-archive` branch preserves previous custom AOI work from `newgraph` branch
- **Issue:** [#1](https://github.com/NewGraphEnvironment/bc_climate_anomaly/issues/1) — Re-apply custom AOI support on synced upstream
...
**Data:** ERA5-Land NetCDF files in `ano_clm_data/` — processed externally, committed to git, no automated download pipeline

## Architecture

- `app.R` — Main Shiny application (~3000 lines), maps + time series + trend analysis
- `1_copy_ano_clm_data.R` — Copies .nc files from local `../data_database/` (not automated)
- `2_spatial_trend_cal_fun.R` — Pre-calculated spatial trend functions
- `shapefiles/` — Spatial boundaries (BC, ecoprovinces, ecoregions, ecosections, FWA watersheds, FLP, municipalities)
- `ano_clm_data/` — NetCDF anomaly and climatology rasters
- `bc_mon_climate_report/` — Quarto monthly/seasonal/annual report generation
- `bc_climate_stripes/` — Climate stripe visualizations
- `ecoprovince_average_anomaly/` — Regional zonal statistics

## Region Picker Layers (upstream)

BC, Ecoprovinces, Ecoregions, Ecosections, Major watersheds, FWA watersheds, FLP boundaries, Municipalities

## Custom AOI Workflow (to be re-applied from aoi-archive)

1. Define study area in `aoi/aoi_create.R` (merge FWA watershed groups or use `fwapgr::fwa_watershed_at_measure()`)
2. Output GeoJSON to `shapefiles/`
3. Inject custom AOIs into FWA watershed picker alongside standard groups
4. Automatically available in the Shiny app

**Previous custom AOIs:** Neexdzii Kwah ("Study Area"), Skeena River Watershed
