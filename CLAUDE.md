...
- **Branch:** `newgraph`
- **Fork is behind upstream main** — periodic sync needed
- **Key fork modifications:** Custom AOI upload support for user-defined study areas (beyond standard ecoprovinces/watersheds)
...
**Data:** ERA5-Land NetCDF files in `ano_clm_data/` (128 subdirs)

## Architecture

- `app.R` — Main Shiny application (~3000 lines), maps + time series + trend analysis
- `aoi/aoi_create.R` — Custom AOI creation using `bcdata` and `fwapgr` packages
- `shapefiles/` — Spatial boundaries (BC, ecoprovinces, watersheds, custom study areas as GeoJSON)
- `ano_clm_data/` — NetCDF anomaly and climatology rasters
- `bc_climate_report_html/` — Quarto monthly/seasonal/annual report generation
- `bc_climate_stripes/` — Climate stripe visualizations
- `ecoprovince_average_anomaly/` — Regional zonal statistics

## Custom AOI Workflow (Fork Feature)
...
3. Custom AOIs combined with standard watershed groups in `fwa_watershed_groups_poly.geojson`
4. Automatically available in the Shiny app watershed picker

**Current custom AOIs:** Neexdzii Kwah ("Study Area"), Skeena River Watershed
